...
const MOVES = {
  ...
  tatsu:{ name:'旋風腳',  seqR:[[2,1,4]], action:'K', altR:[[2,5,4],[2,4,4],[2,2,4]] }
};

function mirrorDir(dir){ const m={1:3,3:1,4:6,6:4,7:9,9:7}; return m[dir] ?? dir; }
...
export function getActiveSeqs(){
  ...
  const alts = (DOM.hitboxMode.checked && mv.altR) ? mv.altR.map(s=>s.slice()) : [];
  let seqs = base.concat(alts);
  if (DOM.face.checked){ seqs = seqs.map(mirrorSeq); }
